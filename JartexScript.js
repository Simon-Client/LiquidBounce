...
script.registerModule({
    ...
    settings: {
		Mode: Setting.list({
			name: "Mode",
			default: "MatrixBoat",
			values: ["MatrixBoat", "MatrixGlide", "JartexOld"]
		}),
		Y: Setting.float({
			name: "BoatY",
			default: 0.65,
...
script.registerModule({
    ...
    settings: {
	    Mode: Setting.list({
			name: "Mode",
			default: "Exploit",
			values: ["Exploit", "Slab", "Old", "Motion"]
		}),
	    Sex: Setting.boolean({
    ...
    if (module.settings.Mode.get() == "Motion") {
    if (packet instanceof HentaiPacket) {
    if (packet.getChatComponent().getUnformattedText().contains(x[parseInt(Math.random()*x.length)])) {
    posSeletced = true;
    }
    }	
    }
    ...
    Chat.print("§8§l[§c§lTPAddon§8§l]");
    Chat.print("§8§l[§c§lTPAddon§8§l]§8 Ladder§7,§8 Web§7,§8 Boat");
    Chat.print("§8§l[§c§lTPAddon§8§l]");
    Chat.print("§8§l[§c§lTPAddon§8§l]§8 --");
    }	
    ...
    if (Teleport.getState() == true && posSelected == false) { 	
    mc.thePlayer.posX = getX;
    mc.thePlayer.posZ = getZ;
    }
    	
    
    if (posSelected == true && mc.gameSettings.keyBindSneak.pressed) {
    mc.thePlayer.sendQueue.addToSendQueue(new C04(mc.thePlayer.posX, mc.thePlayer.posY + 15, mc.thePlayer.posZ, false));
    mc.thePlayer.sendQueue.addToSendQueue(new C04(mc.thePlayer.posX, mc.thePlayer.posY + 1, mc.thePlayer.posZ, true));
    ...
    if (posSelected == true && mc.thePlayer.posX != getX && mc.thePlayer.posZ != getZ) {
    posSelected = false;
    Teleport.setState(false);
    }
    }
